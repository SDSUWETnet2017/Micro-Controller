/* Jeremey Lee
 *
 * Blink LED w/ timer and interrupts
 *
 */
...
void main(void)
{
    ...
    T2CONbits.T2CKPS0 = 0; //Set prescaler to 16
    T2CONbits.T2CKPS1 = 1;
    PR2 = 0xFF;
    ...
    */
    ...
    //turn on
    T1CONbits.TON = 1;
    T1CONbits.T1ECS1 = 1; //use LPRC as clock source
    T1CONbits.T1ECS0 = 0;
    
    T1CONbits.TCKPS1 = 1; //prescaler to 1:256
    T1CONbits.TCKPS0 = 1;
    
    T1CONbits.TCS = 0; //clock select bit, (Fosc/2) or T1ECS<1:0>
    T1CONbits.TGATE = 0; //????
    PR1 = 0x1EFF; //period counter
